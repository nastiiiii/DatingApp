...
public class MessageRepo(DataContext context, IMapper mapper) : IMessageRepo
{
    ...
    {
       var query = context.Messages.OrderByDescending(x => x.MessageSentDate).AsQueryable();
    ...
    public async Task<Group?> GetGroupForConnection(string connectionId)
    {
        return await context.Groups.Include(x => x.Connections)
            .Where(x => x.Connections.Any(y => y.ConnectionId == connectionId)).FirstOrDefaultAsync();
    }
}